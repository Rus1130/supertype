...
| `[instant on\|off]`                        | Toggles `instant`. `on` and `off` can be used to verbosely set the value of `instant`.                               |
| `[ignore on\|off]`                         | Toggles whether or not custom delays are ignored. `on` and `off` can be used to verbosely set the value of `ignore`. |
